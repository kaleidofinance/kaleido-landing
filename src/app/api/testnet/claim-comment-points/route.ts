...
export async function POST(req: Request) {
  try {
    ...
    const newBalance = currentBalance + COMMENT_POINTS;

    // Prevent negative balance
    if (newBalance < currentBalance) {
      console.error(`Prevented balance decrease for ${wallet}. Current: ${currentBalance}, Update: ${COMMENT_POINTS}, New: ${newBalance}`);
      return NextResponse.json({ 
    ...
    const result = await collection.updateOne(
      ...
      { 
        $set: { 
          ...
          twitterCommentTaskClaimed: true
        }
      }
    );
